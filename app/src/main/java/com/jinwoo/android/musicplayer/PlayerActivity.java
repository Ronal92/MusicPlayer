...
public class PlayerActivity extends AppCompatActivity {
    ...
    ViewPager viewPager;
    ImageButton btnPlay, btnFf, btnRew;
    ...
    ViewPager.OnPageChangeListener viewPageListener = new ViewPager.OnPageChangeListener(){
        ...
        @Override
        public void onPageSelected(int position) {  // 바뀐 포지션이 넘어온다.
    ...
    private void next(){
        if(position > 0)
            viewPager.setCurrentItem(PlayerActivity.this.position+1);
    }
        ...
        public void run() {
            while (playStatus < STOP) {
                if(player != null) {
                    ...
                    runOnUiThread(new Runnable() {     // 메인 쓰레드에게 UI 메세지를 던진다.
                        @Override
                        public void run() {
